const EOL = require('os').EOL //EOL: end of line
...
let getCharSet = (rule) => {
    return "@" + rule.type + " " + rule[rule.type] + ";" + EOL
}

let getRule = (rule, indentSpace, indentModule) => {
    let res = indentSpace + getSelectors(rule.selectors) + " {" + EOL + getDeclarations(rule.declarations, indentSpace + indentModule, indentModule) + "}"
    return res
}
...
let getDeclaration = (declaration) => {
    return declaration.property + ": " + declaration.value + ";" + EOL
}
